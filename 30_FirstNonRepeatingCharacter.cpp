...
int findIndex(const std::string& inString) {
    ...
    for(int i = 0; i < inString.size(); ++i) {
        ...
        auto findResult = std::find_if(seenChars.begin(), seenChars.end(), 
            [character = inString[i]](const std::pair<char,int>& currentPair ){
    ...
    auto result = std::min_element(seenChars.begin(), seenChars.end(),
